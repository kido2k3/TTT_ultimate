...
#     score_micro_current = evaluate_local_board(state.blocks[local_board_check], state.X, state.O)
#     score_micro_previous = evaluate_local_board(prev_state.blocks[local_board_check], state.X, state.O)

#     # print("Delta", score_micro_current - score_micro_previous)
#     # exit()
#     score_macro_current = evaluate_local_board(state.global_cells.reshape(3, 3), state.X, state.O)
#     score_macro_previous = evaluate_local_board(prev_state.global_cells.reshape(3, 3), state.X, state.O)
...
#     else:
#         Rt = (score_micro_current - score_micro_previous) * (score_macro_current - score_macro_previous)
#     # print(Rt)
...
# def minimax(state, prev_state, depth, cumulated_reward = 0, maximize_player = False):
...
#             eval, _ = minimax(state_copy, prev_state, newDepth, cumulated_reward + reward, False)
#             if eval > best_score:
#                 best_score = eval
#                 best_move = move
...
#             state_copy.act_move(move)
#             reward = evaluate(state_copy, prev_state)
#             eval, _ = minimax(state_copy, prev_state, newDepth, cumulated_reward + reward, True)
#             if eval < best_score:
#                 best_score = eval
#                 best_move = move
#         return best_score, best_move
...
#     depth = 4
#     eval, best_move = minimax(cur_state, cur_state, depth)
...
def select_move(cur_state, remain_time):
    ...
    if len(valid_moves) != 0:
        # for i in range(len(cur_state.get_valid_moves)):
        #     print(f"{i}: {cur_state.get_valid_moves[i]}")
        # temp = int(input("nhap i:"))
        # return cur_state.get_valid_moves[temp]
